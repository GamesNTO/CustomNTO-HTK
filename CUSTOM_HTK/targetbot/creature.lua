...
UI.Button("Creature List", function(newText)
  UI.MultilineEditorWindow(storage.creatureList or '', {title="Creature List", description="Add the name of the monsters you don't want to attack.\n Example: \nwolf \ndinosaur", width = 200, height = 100}, function(text)
      storage.creatureList = text;
  end)
end)
...
TargetBot.Creature.getConfigs = function(creature)
  if not creature then return {} end
  local name = creature:getName():trim():lower();
  if storage.creatureList then
    local ignoreList = string.split(storage.creatureList, "\n");
